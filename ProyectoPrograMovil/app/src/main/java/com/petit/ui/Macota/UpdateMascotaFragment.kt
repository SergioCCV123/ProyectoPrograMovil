...
class UpdateMascotaFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        if(args.mascota.rutaAudio?.isNotEmpty()== true){
            mediaplayer = MediaPlayer()
            mediaplayer.setDataSource(args.mascota.rutaAudio)
            mediaplayer.prepare()
        ...
        if(args.mascota.rutaImagen?.isNotEmpty()== true){
            Glide.with(requireContext())
                .load(args.mascota.rutaImagen)
                .fitCenter()
                .into(binding.imagen)
        }
